...
outdir=/data/research/NGS/data/clinical_exome_rappresentative/bam_hg38
sample_list=/data/research/NGS/data/clinical_exome_rappresentative/fastq/sample_list.list
...
for idsample in `cat $sample_list`; do
    ...
    if [ -d "$dir" ]; then
        ...
        #3. mapping
        echo "> mapping bwa "
        bwa mem -t 20 -R "@RG\tID:$idsample\tSM:$idsample" $genome $idsample_R1 $idsample_R2 |  samtools view -b - > $outdir/$idsample/$idsample.raw.bam
        #4. View
        #singularity exec -B $mount_dir_singu $biotools samtools view -h $outdir/$idsample/$idsample.raw.bam | less -S
        #5. 
        echo "> sambamba sort "
        sambamba sort -t 20 -m 25G --tmpdir $tmpdir -o $outdir/$idsample/${idsample}.raw.sorted.bam $outdir/$idsample/${idsample}.raw.bam
        #6. mark duplicates
        echo "> sambamba remove duplicates "
        sambamba markdup -t 20 -p --tmpdir $tmpdir --overflow-list-size 500000 $outdir/$idsample/${idsample}.raw.sorted.bam $outdir/$idsample/${idsample}.bam
        #7.
    fi
done